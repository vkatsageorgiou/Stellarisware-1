...
unsigned short g_usVendorID  = 0x1CBE;    // Luminary Micro
unsigned short g_usProductID = 0x00FF;    // Luminary Micro DFU boot loader
...
//*****************************************************************************
//
// The Luminary Micro -specific binary image suffix used by the DFU driver to
// determine where the image should be located in flash.
//
//*****************************************************************************
unsigned char g_pcDFUPrefix[] =
{
    0x01, // LUMINARY_DFU_PROG
    0x00, // Reserved
...
PrintWelcome(void)
{
    ...
    QUIETPRINT("Copyright (c) 2008 Luminary Micro, Inc.  All rights reserved.\n\n");
}
...
// it for the DFU suffix and Luminary prefix.
//
...
// Luminary-specific DFU download prefix structure.
//
//*****************************************************************************
BOOL
...
IsSuffixValid(unsigned char *pcData, unsigned char *pcEnd)
{
    ...
    {
        VERBOSEPRINT("Read CRC 0x%08x, calculated 0x%08x.\n", ulCRCRead, ulCRCCalc);
...
//*****************************************************************************
//
// Dump the contents of the Luminary-specfic DFU file prefix.
//
//*****************************************************************************
void
DumpPrefix(unsigned char *pcPrefix)
{
    ...
    QUIETPRINT("Payload length: %d (0x%x) bytes, %dKB\n", ulLength, ulLength,
               ulLength / 1024);
}

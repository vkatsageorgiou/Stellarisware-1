//
// lmdfu.cpp : Defines the initialization routines for the DLL.
//
...
FindDescriptor(unsigned char ucType, unsigned char *pcStart, unsigned long ulLength)
{
    unsigned char *pcEnd;
...
//****************************************************************************
//
// Query the DFU device information structure (for Luminary-compatible
// devices).
//
//****************************************************************************
static tLMDFUErr
DFUDeviceInfoGet(tLMDFUDeviceState *pState, tDFUDeviceInfo *psDevInfo)
{
    ...
    sInfoCmd.ucCommand = LUMINARY_CMD_INFO;
...
//****************************************************************************
//
// Determine whether the connected device supports Luminary's DFU binary
// protocol and, if so, query the device information structure and fill in
// various fields in the state structure.
//
//****************************************************************************
static bool
CheckForLuminaryProtocol(tLMDFUDeviceState *pState)
{
    int iRetcode;
    tLMDFUErr eRetcode;
    tDFUQueryLuminaryProtocol sProt;
    tDFUDeviceInfo sDevInfo;

    //
    // Clear out the Luminary-specific fields in the state structure.
    //
    ...
    //
    iRetcode = usb_control_msg( pState->hUSB,
          USB_ENDPOINT_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE,
          USBD_DFU_REQUEST_LUMINARY,
          REQUEST_LUMINARY_VALUE,
          pState->usInterface,
          (char *)&sProt,
    ...
    //
    if((iRetcode == sizeof(tDFUQueryLuminaryProtocol)) &&
       (sProt.usMarker == LM_DFU_PROTOCOL_MARKER) &&
       (sProt.usVersion == LM_DFU_PROTOCOL_VERSION_1))
    {
        //
        // We can safely use Luminary extensions when talking to this device.
        //
...
//****************************************************************************
static unsigned long
MapPartInfo(unsigned long ulPartInfo)
{
    int iLoop;
    unsigned char ucPartNo;
    ...
    //
    ucPartNo = (unsigned char)((ulPartInfo & LUMINARY_PART_M) >>
                               LUMINARY_PART_SHIFT);
    ...
    for(iLoop = 0; iLoop < NUM_STELLARIS_PARTS; iLoop++)
    {
        ...
        {
            //
            // We found it - return the part number.
            //
...
GetPartNumberAndRev(tLMDFUDeviceState *pState, tLMDFUDeviceInfo *pDevInfo)
{
    ...
    // Luminary protocol or not.
    //
    pDevInfo->bSupportsLuminaryExtensions = pState->bSupportsLMProtocol;
...
//****************************************************************************
//
// Maps a return code from a libusb-win32 function into the equivalent value
// in tLMDFUErr (assuming one exists).
//
//****************************************************************************
static tLMDFUErr
MapLIBUSBRetcode(int iRetcode)
{
    switch(iRetcode)
    {
        case -EPIPE:
                    ...
                    unsigned char *pcData, int iLength)
{
    ...
    if(iRetcode != iLength)
    {
        TRACE("Error %d from usb_control_msg\n", iRetcode);
        return(MapLIBUSBRetcode(iRetcode));
    }
    ...
    if(bCheckStatus)
    {
        ...
        do
        {
            ...
            if(iRetcode != sizeof(tDFUGetStatusResponse))
            {
                TRACE("Error %d from usb_control_msg\n", iRetcode);
                return(MapLIBUSBRetcode(iRetcode));
            }
...
LMDFUInit(void)
{
    ...
    //
    // Initialize libusb-win32 if we have not already done this.
    //
    if(!bInitialized)
    {
        usb_init();
                ...
                tLMDFUHandle *phHandle)
{
    ...
    struct usb_config_descriptor *pConfig;
    ...
    //
    // Find all the USB busses and devices on this system.
    //
    usb_find_busses();
    usb_find_devices();

    //
    // Get the linked list of buses found.
    //
    pBus = usb_get_busses();

    //
    // Now search the devices for any offering a DFU interface.
    //
    while(pBus)
    {
        //
        // Get the list of devices on this bus.
        //
        ...
        {
            //
            // Look at each of this device's configurations.
            //
            for(iConfigCount = 0;
                iConfigCount < pDevice->descriptor.bNumConfigurations;
                iConfigCount++)
            {
                //
                // Get a pointer to the configuration descriptor
                //
                pConfig = &(pDevice->config[iConfigCount]);

                //
                // Now look at each of the interfaces supported by this
                // configuration.
                //
                for(iIntCount = 0;
                    iIntCount < pConfig->interface->num_altsetting;
                    iIntCount++)
                {
                    //
                    // Get a pointer to the interface.
                    //
                    pInterface = &(pConfig->interface->altsetting[iIntCount]);

                    //
                    // Check to see if this interface has the required class
                    // and subclass.
                    //
                    if((pInterface->bInterfaceClass == USB_CLASS_APP_SPECIFIC)
                        && (pInterface->bInterfaceSubClass == USB_DFU_SUBCLASS))
                    {
                        //
                        // We have found a DFU interface.  Is this the one we
                        // were asked to return?
                        //
                        if(iCount == iDeviceIndex)
                        {
                            //
                            // Fill in the supplied device info structure
                            //
                            psDevInfo->usVID = pDevice->descriptor.idVendor;
                            psDevInfo->usPID = pDevice->descriptor.idProduct;
                            psDevInfo->usDevice = pDevice->descriptor.bcdDevice;
                            psDevInfo->ucManufacturerString =
                                pDevice->descriptor.iManufacturer;
                            psDevInfo->ucProductString =
                                pDevice->descriptor.iProduct;
                            psDevInfo->ucSerialString =
                                pDevice->descriptor.iSerialNumber;
                            psDevInfo->ucDFUInterfaceString =
                                pInterface->iInterface;
                            psDevInfo->bDFUMode =
                             ((pInterface->bInterfaceProtocol == DFU_PROTOCOL) ?
                                             true : false);

                            //
                            // Get the DFU functional descriptor.
                            //
                            pDFU = FindDescriptor(USB_DESC_TYPE_DFU,
                                                  pInterface->extra,
                                                  pInterface->extralen);
                            ...
                            if(pDFU)
                            {
                                ...
                                //
                                psDevInfo->usDetachTimeOut =
                                    DFU_DETACHTIMEOUT(pDFU);
                                psDevInfo->usTransferSize =
                                    DFU_MAXTRANSFER(pDFU);
                                psDevInfo->ucDFUAttributes =
                                    DFU_ATTRIBUTES(pDFU);
                            }
                            else
                            {
                                //
                                // This naughty DFU device doesn't publish a
                            ...
                            // Now open the device.
                            //
                            pState->hUSB = usb_open(pDevice);

                            //
                            // Did we open the device successfully?
                            //
                            if(!pState->hUSB)
                            {
                                //
                                // Oops - tell the caller something went wrong.
                                //
                                return(DFU_ERR_UNKNOWN);
                            }
                            else
                            {
                                //
                                // Remember various important pieces of info
                                // about the device then tell the caller we
                                // found it, giving them back the handle.
                                //
                                pState->usVID = psDevInfo->usVID;
                                pState->usPID = psDevInfo->usPID;
                                pState->usTransferSize =
                                    psDevInfo->usTransferSize;
                                pState->ucDFUAttributes =
                                    psDevInfo->ucDFUAttributes;
                                pState->usInterface =
                                ...
                                //
                                // Make sure the device is idle before we do
                                // anything that may be illegal in various
                                // error states.
                                //
                                DFUMakeDeviceIdle(pState);

                                //
                                // Determine whether this device supports the
                                // Luminary-specific binary protocol and, if so,
                                // read the device information structure.
                                //
                                CheckForLuminaryProtocol(pState);

                                //
                                // If possible, determine the specific
                                // Stellaris part we are talking to.
                                //
                        ...
                        {
                            //
                            // This is not the device we are looking for so
                            // just bump our "found" count and keep looking.
                            //
                            iCount++;
                        }
                    }
        ...
        //
        // Move on to the next bus.
        //
        pBus = pBus->next;
    }

    //
    // If we drop out of the loop, we didn't find the requested device.
    //
...
LMDFUDeviceClose(tLMDFUHandle hHandle, bool bReset)
{
    ...
    if(bReset)
    {
        ...
        // Yes - do it.
        //
        usb_reset(pState->hUSB);
    }

    //
    // Close the device handle.
    //
    iRetcode = usb_close(pState->hUSB);
                     ...
                     unsigned short *pusStringLen)
{
    ...
    //
    // Call libusb-win32 to get the requested string.
    //
    iRetcode = usb_get_string(pState->hUSB, ucStringIndex, usLanguageID,
                              pcString, (int)*pusStringLen);

    //
    // If the string was read, update the length to indicate the number of
    // bytes read.
    //
    if(iRetcode >= 0)
    {
        *pusStringLen = (unsigned short)iRetcode;
        return(DFU_OK);
    }
    else
    {
        //
        // We couldn't read the string!
        //
        return(DFU_ERR_NOT_FOUND);
    }
}
                                              ...
                                              unsigned short *pusStringLen)
{
    ...
    // Call libusb-win32 to get the requested string.
    //
    iRetcode = usb_get_string_simple(pState->hUSB, ucStringIndex,
                                     pcString, (int)*pusStringLen);
...
//!
//! This function makes use of a Luminary Micro-specific extension to the
...
//! \e pcDFUImage.
//! \param pbLuminaryFormat is a pointer which will be written to \e true if
//! the supplied data appears to start with a valid Luminary Micro DFU prefix.
//! If the data ends in a valid DFU suffix structure but does not contain the
//! Luminary prefix, this value will be written to \e false.
//!
...
//! presence of a Luminary Micro DFU prefix.  This structure contains the
//! address at which to flash the image and also the length of the payload.
//! The pbLuminaryFormat pointer is written to \e true if the following
...
//!    the suffix itself) and Luminary prefix (8 bytes).
...
LMDFUIsValidImage(tLMDFUHandle hHandle, unsigned char *pcDFUImage,
                   unsigned long ulImageLen, bool *pbLuminaryFormat)
{
    tLMDFUDeviceState *pState;
    unsigned long ulCRC;
    ...
    if(!pcDFUImage || !pbLuminaryFormat)
    {
        return(DFU_ERR_INVALID_ADDR);
    }
    ...
    // device we intend using it with.  Does it have a Luminary prefix?  It
    ...
    {
        *pbLuminaryFormat = false;
    }
    else
    {
        *pbLuminaryFormat = true;
    }
...
//! Luminary Micro-specific header informing the device of the address to
...
//!
//! To flash a pure binary image without the DFU suffix or Luminary Micro
//! prefix, use LMDFUDownloadBin() instead of this function.
//!
//! Note that the Luminary-specific prefix structure contains the address at
//! which the image is to be flashed so no parameter exists here to provide
//! this information.
//!
//! This function uses no Luminary-specific extensions to the DFU protocol and
...
//! Luminary Micro extensions, the binary image will be downloaded to the
...
//! Luminary Micro header to the device at an address supplied by the caller.
//!
//! This function is synchronous and will not return until the operation is
//! complete.  To receive periodic status updates during the operation, a
//! window handle may be provided.  This window will receive WM_LMDFU_STATUS
                 ...
                 bool bVerify, HWND hwndNotify)
{
    ...
    //
    // We only support binary downloads for Luminary Micro targets since we
    // need to use a protocol extension to tell the board where to flash the
    // image.
    //
    if(!pState->bSupportsLMProtocol)
    {
        TRACE("Target does not support Luminary DFU protocol.\n");
        return(DFU_ERR_UNSUPPORTED);
    }
    ...
    // image.
    //
    sProg.ucCommand = LUMINARY_CMD_PROG;
    sProg.ucReserved = 0;
    ...
    if((eRetcode == DFU_OK) && bVerify)
    {
        ...
        if((eRetcode == DFU_OK) && (sStatus.bState == STATE_IDLE))
        {
            ...
            sBin.ucCommand = LUMINARY_CMD_BIN;
            sBin.bBinary = 1;
            ...
            if(eRetcode == DFU_OK)
            {
                ...
                sRead.ucCommand = LUMINARY_CMD_READ;
           ...
           unsigned long ulEraseLen, bool bVerify, HWND hwndNotify)
{
    ...
    // Luminary Micro protocol.
    //
    if(!pState->bSupportsLMProtocol)
    {
        TRACE("Device does not support Luminary Micro protocol.\n");
        return(DFU_ERR_UNSUPPORTED);
    }
    ...
    //
    sErase.ucCommand = LUMINARY_CMD_ERASE;
    sErase.ucReserved = 0;
    ...
    if((eRetcode == DFU_OK) && bVerify)
    {
        ...
        //
        sCheck.ucCommand = LUMINARY_CMD_CHECK;
        sCheck.ucReserved = 0;
...
//! Luminary-specific prefix and DFU standard suffix. If \e false, the wrappers
    ...
    if(eRetcode == DFU_OK)
    {
        ...
        if(eRetcode == DFU_OK)
        {
            ...
            if(eRetcode == DFU_OK)
            {
                ...
                // to the length of data we request via the LUMINARY_CMD_READ

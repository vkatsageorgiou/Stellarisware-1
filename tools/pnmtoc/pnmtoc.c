           ...
           unsigned long ulHeight, unsigned long ulLength)
{
    ...
    if(g_ulNumColors > 2)
    {
        ...
        for(ulIdx = 0; ulIdx < g_ulNumColors; ulIdx++)
        {
            printf("    0x%02x, 0x%02x, 0x%02x,\n", g_pulColors[ulIdx] & 255,
                   (g_pulColors[ulIdx] >> 8) & 255,
...
// the Luminary Micro Graphics Library.
//
//*****************************************************************************
int
main(int argc, char *argv[])
{
    unsigned long ulLength, ulWidth, ulHeight, ulMax, ulCount;
    ...
    for(ulCount = 0, ulMax = 2; ulCount < 3; )
    {
        ...
        {
            if(sscanf(pucData + ulMax, "%u", &ulCount) != 1)
            {
    ...
    //
    GetNumColors(pucData + ulMax, ulWidth, ulHeight);
    ...
    if(g_ulNumColors <= 2)
    {
        ...
        // pixel.
        //
        ulLength = Encode1BPP(pucData + ulMax, ulWidth, ulHeight);
    }
    else if(g_ulNumColors <= 16)
    {
        //
        ...
        // per pixel.
        //
        ulLength = Encode4BPP(pucData + ulMax, ulWidth, ulHeight);
    }
    else if(g_ulNumColors <= 256)
    {
        ...
        // bits per pixel.
        //
        ulLength = Encode8BPP(pucData + ulMax, ulWidth, ulHeight);
    }
    else
    {
        //

...
ShowHelp(void)
{
    ...
    printf("This application may be used to download images to a Luminary\n");
...
DumpDeviceInformation(tLMDFUHandle hHandle, tLMDFUDeviceInfo *psDevInfo)
{
    ...
    QUIETPRINT("LM Extensions: %s\n", psDevInfo->bSupportsLuminaryExtensions ?
                    "Supported" : "Not Supported");
    if(psDevInfo->bSupportsLuminaryExtensions)
    {
        QUIETPRINT("Target:        LM3S%4x revision %c%c\n",
                   psDevInfo->ulPartNumber,
...
UploadImage(tLMDFUHandle hHandle, tLMDFUParams *psDFU)
{
    ...
    //
    // We only support upload on devices supporting the Luminary Micro DFU
    // protocol since, on other devices, we have no idea how large an
    // uploaded image to expect.
    //
    if(!psDFU)
    {
        QUIETPRINT("Target device does not support Luminary Micro protocol.\n");
        return(40);
    }
...
DownloadImage(tLMDFUHandle hHandle, tLMDFUParams *psDFU)
{
    ...
    //
    eRetcode = _LMDFUIsValidImage(hHandle, pcFileBuf, (unsigned long)iLen, &bLuminaryFormat);
       ...
       bLuminaryFormat)
    {
        ...
        // Since it also contains a Luminary prefix, we can send it to the
        // main download function.
        //
        VERBOSEPRINT("Image contains valid DFU suffix and Luminary prefix.\n");
    ...
    else
    {
        ...
        //
        // Did the file contain a DFU suffix but no Luminary prefix? If so,
...
main(int argc,char* argv[])
{
    ...
    if(eRetcode != DFU_OK)
    {
        ...
        if(eRetcode == DFU_ERR_NOT_FOUND)
        {
            ...
            printf("\"Luminary Micro embedded USB drivers\" which may be downloaded from\n");
            printf("http:\\\\www.luminarymicro.com/products/software_updates.html.\n\n");
            iExitCode = 10;
        }
        else if(eRetcode == DFU_ERR_INVALID_ADDR)
        {
            ...
            printf("install the latest device driver and example applications from the Luminary\n");
            printf("Micro web site to ensure that you are using compatible versions. The drivers\n");
            printf("can be found in the package named \"Luminary Micro embedded USB drivers\" and\n");
            printf("the applications can be found in package \"Windows-side examples for USB kits\"\n");
            printf("both of which may be downloaded from\n");
            printf("http:\\\\www.luminarymicro.com/products/software_updates.html.\n\n");
            iExitCode = 11;
        }
        else
        {
            ...
            printf("and reinstall the latest device driver and example applications from the Luminary\n");
            printf("Micro web site. The drivers can be found in the package named \"Luminary Micro\n");
    ...
    do
    {
        ...
        if(eRetcode == DFU_OK)
        {
            ...
            {
                //
                // We have been asked to clear a section of the flash or
                // upload or download an image.  Is this the device we have
                // been asked to use?
                //
                if(iDevIndex == g_iDeviceIndex)
                {
                    ...
                    //
                    // If it supports the Luminary DFU protocol, read back
                    // some important parameters.
                    //
                    if(sDevInfo.bSupportsLuminaryExtensions)
                    {
                        //
                    ...
                    {
                        //
                        // The device doesn't support the Luminary Micro
                        // protocol so set the DFU parameter structure pointer
                        // to NULL.
                        //
                    ...
                    if(eRetcode == DFU_OK)
                    {
                        ...
                        {
                            //
                            // Yes - clear the required area of device flash.
                            //
                            iExitCode = ClearFlash(hHandle, psDFU);
                        }
                        //
                        // Yes - have we been asked to upload the current image to
                        // a file on the host?
                        //
                        else if(g_bUpload)
                        {
                        ...
                        {
                            //
                            // No - download an image to the device.
                            //
            ...
            //
            eRetcode = _LMDFUDeviceClose(hHandle, false);

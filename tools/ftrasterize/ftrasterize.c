...
Usage(char *argv)
{
    fprintf(stderr, "Usage: %s [-b] [-f <filename>] [-i] [-s <size>] <font>\n",
            basename(argv));
    fprintf(stderr, "Converts any font that FreeType recognizes into a "
                    "compressed font for use by\n");
    fprintf(stderr, "the Luminary Micro Graphics Library.\n");
    fprintf(stderr, "\n");
...
// compressed raster font for use by the Luminary Micro Graphics Library.
//
//*****************************************************************************
int
main(int argc, char *argv[])
{
    ...
    int iPrevBit, iBit, iZero, iOne, iBold, iItalic;
    ...
    //
    while((iOpt = getopt(argc, argv, "bf:is:")) != -1)
    {
        //
    ...
    for(uiChar = 1; uiChar < 0x5f; uiChar++)
    {
                         ...
                         FT_LOAD_DEFAULT | FT_LOAD_TARGET_MONO) == 0)
        {
            ...
            //
            g_pGlyphs[uiChar].width = ppMap->width;
            g_pGlyphs[uiChar].rows = ppMap->rows;
            g_pGlyphs[uiChar].pitch = ppMap->pitch;
            g_pGlyphs[uiChar].bitmap_top = pSlot->bitmap_top;
    ...
    //
    // Loop through the glyphs, finding the minimum and maximum Y values for
    // the glyphs.
    //
    for(pGlyph = g_pGlyphs, iYMin = 0, iYMax = 0; pGlyph < (g_pGlyphs + 95);
        pGlyph++)
    {
        if(pGlyph->bitmap_top > iYMin)
        {
            iYMin = pGlyph->bitmap_top;
        }
        if((pGlyph->bitmap_top - pGlyph->rows + 1) < iYMax)
        {
            iYMax = pGlyph->bitmap_top - pGlyph->rows + 1;
        }
    }
    ...
    //
    for(pGlyph = g_pGlyphs, uiChar = 32, iWidth = 0;
        pGlyph != (g_pGlyphs + 0x5f); pGlyph++, uiChar++)
    {
        //
        // Find the minimum and maximum X value for this glyph.  Loop through
        // the rows of the bitmap for this glyph.
        //
        for(iY = 0, iXMin = 1000000, iXMax = 0; iY < pGlyph->rows; iY++)
        {
            //
            // Find the minimum X for this row of the glyph.
            //
            for(iX = 0; iX < pGlyph->width; iX++)
            {
                if(pGlyph->pucData[(iY * pGlyph->pitch) + (iX / 8)] &
                   (1 << (7 - (iX & 7))))
                {
                    if(iX < iXMin)
                    {
            ...
            iY < (iYMin - iYMax + 1); iY++)
        {
            ...
            // necessary.
            //
            for(iX = 0; iX < (iXMax - iXMin + 1 + (iSize / 10)); iX++)
            {
                //
                ...
                //
                if((iY >= (iYMin - pGlyph->bitmap_top)) &&
                   (iY < (iYMin - pGlyph->bitmap_top + pGlyph->rows)) &&
                   (iX < (iXMax - iXMin + 1)))
                {
                    //
                    // Extract this bit from the bitmap data for this glyph.
                    //
                    if(pGlyph->pucData[((iY - iYMin + pGlyph->bitmap_top) *
                                        pGlyph->pitch) + ((iX + iXMin) / 8)] &
                       (1 << (7 - ((iX + iXMin) & 7))))
                    {
                        iBit = 1;
                    }
        ...
        pGlyph->pucChar[1] = iXMax - iXMin + 1 + (iSize / 10);

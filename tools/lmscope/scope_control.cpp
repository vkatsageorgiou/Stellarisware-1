...
static DWORD WINAPI ReadConnectThread(LPVOID lpParam)
{
    ...
    while(1)
    {
        ...
        while(!pDevice->bDeviceConnected)
        {
            ...
            // Try to connect.
            //
            pDevice->hUSB = InitializeDevice(SCOPE_VID, SCOPE_PID,
...
bool ScopeControlDisconnect(void)
{
    ...
    if(!devInfo.hwndNotify)
    {
        ...
        {
            bRetcode = TerminateDevice(devInfo.hUSB);
        }
...
bool ScopeControlConnect(tScopeSettings *psSettings)
{
    ...
    if(!devInfo.hwndNotify)
    {
        ...
        //
        devInfo.hUSB = InitializeDevice(SCOPE_VID, SCOPE_PID,
                                  (LPGUID)&(GUID_DEVINTERFACE_LUMINARY_SCOPE),
                                  &bDriverInstalled);
                            ...
                            void *pData)
{
    ...
    if(devInfo.bDeviceConnected)
    {
        ...
        bResult = WriteUSBPacket(devInfo.hUSB, (unsigned char *)&sPacket,
        ...
        else
        {
            ...
            if(ulDataLength != 0)
            {
                bResult = WriteUSBPacket(devInfo.hUSB, (unsigned char *)pData,
                            ...
                            void **ppData)
{
    ...
    if(devInfo.bDeviceConnected)
    {
        dwResult = ReadUSBPacket(devInfo.hUSB, (unsigned char *)&sPacket,
                ...
                (sPacket.ucHdrLength == sizeof(tScopePacket)))
            {
                ...
                if(sPacket.ulDataLength != 0)
                {
                    ...
                    *ppData = LocalAlloc(LMEM_FIXED, sPacket.ulDataLength);
                    ...
                    // Get the data
                    //
                    dwResult = ReadUSBPacket(devInfo.hUSB,
                                     ...
                                     DWORD dwTimeoutmS)
{
    ...
    //
    dwError = ReadUSBPacket(pDevice->hUSB, pDest, dwSize, pdwCountRead,

...
//
//*****************************************************************************
#define USB_BASE_TO_INDEX(BaseAddr) \
                                0
#define USB_INDEX_TO_BASE(Index) \
                                USB0_BASE
...
#define USB_INT_IN              USB_INTEP_DEV_IN_3

#define INT_OUT_ENDPOINT        USB_EP_3
#define USB_INT_OUT             USB_INTEP_DEV_OUT_3
...
//
//*****************************************************************************
static const tUSBDHIDDevice *g_ppsHIDDevices[1];
...
//*****************************************************************************
//
// The HID config descriptor is defined as four or five sections depending upon
// the client's configuration choice.  These sections are:
//
...
//*****************************************************************************
//
// Placeholder for the user's HID descriptor block.
//
//*****************************************************************************
...
// together to generate the config descriptor.  Note that this must be in RAM
// since we need to include or exclude the final section based on client-
// supplied initialization parameters.
//
//*****************************************************************************
...
//
//*****************************************************************************
static void HandleGetDescriptor(unsigned long ulIndex,
                                tUSBRequest *pUSBRequest);
static void HandleRequest(unsigned long ulIndex,
                          tUSBRequest *pUSBRequest);
static void HandleConfigChange(unsigned long ulIndex, unsigned long ulInfo);
static void HandleEP0DataReceived(unsigned long ulIndex, unsigned long ulInfo);
static void HandleEP0DataSent(unsigned long ulIndex, unsigned long ulInfo);
static void HandleReset(unsigned long ulIndex);
...
ScheduleReportTransmission(tHIDInstance *psInst)
{
    ...
    //
    iRetcode = USBEndpointDataPut(psInst->ulUSBBase, INT_IN_ENDPOINT, pucData,
                                  ulNumBytes);
    ...
    if(iRetcode != -1)
    {
        ...
        //
        iRetcode = USBEndpointDataSend(psInst->ulUSBBase, INT_IN_ENDPOINT,
                                           USB_TRANS_IN);
    }
...
ProcessDataFromHost(const tUSBDHIDDevice *psDevice, unsigned long ulStatus)
{
    ...
    //
    ulEPStatus = USBEndpointStatus(USB0_BASE, INT_OUT_ENDPOINT);

    //
    // Clear the status bits.
    //
    USBDevEndpointStatusClear(USB0_BASE, INT_OUT_ENDPOINT, ulEPStatus);
...
ProcessDataToHost(const tUSBDHIDDevice *psDevice, unsigned long ulStatus)
{
    ...
    // Get the endpoint status to see why we were called.
    //
    ulEPStatus = USBEndpointStatus(psInst->ulUSBBase, INT_IN_ENDPOINT);

    //
    // Clear the status bits.
    //
    USBDevEndpointStatusClear(psInst->ulUSBBase, INT_IN_ENDPOINT, ulEPStatus);
...
HandleEndpoints(unsigned long ulIndex, unsigned long ulStatus)
{
    ...
    if(ulStatus & USB_INT_OUT)
    {
        ...
        //
        ProcessDataFromHost(g_ppsHIDDevices[ulIndex], ulStatus);
    ...
    {
        ProcessDataToHost(g_ppsHIDDevices[ulIndex], ulStatus);
        ulStatus &= ~USB_INT_IN;
    }
}
...
HandleConfigChange(unsigned long ulIndex, unsigned long ulInfo)
{
    ...
    // Get a pointer to our instance data.
    //
    psInst = g_ppsHIDDevices[ulIndex]->psPrivateHIDData;
    ...
    if(!psInst->bConnected)
    {
        ...
        //
        g_ppsHIDDevices[ulIndex]->pfnRxCallback(
                                   g_ppsHIDDevices[ulIndex]->pvRxCBData,
    ...
    //
    ClearIdleTimers(g_ppsHIDDevices[ulIndex]);
...
//*****************************************************************************
static void
HandleGetDescriptor(unsigned long ulIndex, tUSBRequest *pUSBRequest)
{
    unsigned long ulSize;
    unsigned long ulDesc;
    const tUSBDHIDDevice *psDev;
    ...
    switch(pUSBRequest->wValue >> 8)
    {
        ...
        case USB_HID_DTYPE_PHYSICAL:
        {
            ...
            //
            ulSize = 0;
            ulDesc = FindHIDDescriptor(psDev, pUSBRequest->wValue >> 8,
            ...
            {
                //
                // No - stall the endpoint and reutrn.
                //
                USBDCDStallEP0(0);
                return;
            }
            ...
            //
            USBDCDSendDataEP0(0,
                            (unsigned char *)psDev->ppClassDescriptors[ulDesc],
                            ulSize);
        ...
        //
        // This is a request for the HID descriptor (as found in the config
        // descriptor following the relevant interface).
        //
        case USB_HID_DTYPE_HID:
        {
            ...
            //
            ulSize = (unsigned long)
                        g_ppsHIDDevices[ulIndex]->psHIDDescriptor->bLength;
            ...
            // Send the data via endpoint 0.
            //
            USBDCDSendDataEP0(0, (unsigned char *)
                              g_ppsHIDDevices[ulIndex]->psHIDDescriptor,
                              ulSize);
            break;
        }
...
//*****************************************************************************
static void
HandleRequest(unsigned long ulIndex, tUSBRequest *pUSBRequest)
{
    tHIDInstance *psInst;
    ...
    switch(pUSBRequest->bRequest)
    {
        ...
        case USBREQ_SET_REPORT:
        {
            ...
            psInst->pucOutReportData = (unsigned char *)
                            g_ppsHIDDevices[ulIndex]->pfnRxCallback(
                               g_ppsHIDDevices[ulIndex]->pvRxCBData,
            ...
            else
            {
                ...
                //
                USBDCDRequestDataEP0(ulIndex, psInst->pucOutReportData,
        ...
        case USBREQ_GET_REPORT:
        {
            ...
            //
            ulSize = g_ppsHIDDevices[ulIndex]->pfnRxCallback(
                               g_ppsHIDDevices[ulIndex]->pvRxCBData,
            ...
            USBDCDSendDataEP0(ulIndex, pucReport, ulSize);
        ...
        case USBREQ_SET_IDLE:
        {
            ...
            //
            SetIdleTimeout(g_ppsHIDDevices[ulIndex],
                           pUSBRequest->wValue & 0xFF,
        ...
        case USBREQ_GET_IDLE:
        {
            ...
            //
            ulTimeout = GetIdleTimeout(g_ppsHIDDevices[ulIndex],
                                       pUSBRequest->wValue);
            ...
            if(ulTimeout != HID_NOT_FOUND)
            {
                ...
                //
                USBDCDSendDataEP0(ulIndex, (unsigned char *)&ulTimeout, 1);
            }
            else
            {
                //
        ...
        {
            if(g_ppsHIDDevices[ulIndex]->ucSubclass == USB_HID_SCLASS_BOOT)
            {
                //
        ...
        case USBREQ_GET_PROTOCOL:
        {
            if(g_ppsHIDDevices[ulIndex]->ucSubclass == USB_HID_SCLASS_BOOT)
            {
                ...
                //
                ucProtocol = (unsigned char)
                               g_ppsHIDDevices[ulIndex]->pfnRxCallback(
                                   g_ppsHIDDevices[ulIndex]->pvRxCBData,
                                   USBD_HID_EVENT_GET_PROTOCOL, 0, (void *)0);

                //
                // Send our response to the host.
                //
                USBDCDSendDataEP0(ulIndex, (unsigned char *)&ucProtocol, 1);
            }
            else
            {
                //
...
//*****************************************************************************
static void
HandleEP0DataReceived(unsigned long ulIndex, unsigned long ulDataSize)
{
    tHIDInstance *psInst;
...
//*****************************************************************************
static void
HandleEP0DataSent(unsigned long ulIndex, unsigned long ulInfo)
{
    tHIDInstance *psInst;
...
//*****************************************************************************
static void
HandleReset(unsigned long ulIndex)
{
    //
...
//*****************************************************************************
static void
HIDTickHandler(unsigned long ulIndex, unsigned long ulTimemS)
{
    tHIDInstance *psInst;
    unsigned long ulSize;
    ...
    if(psInst->bConnected)
    {
        ProcessIdleTimers(g_ppsHIDDevices[ulIndex], ulTimemS);
    }
...
USBDHIDInit(unsigned long ulIndex, const tUSBDHIDDevice *psDevice)
{
    ...
    // controller.
    //
    if(g_ppsHIDDevices[ulIndex] != (tUSBDHIDDevice *)0)
    {
        return ((void *)0);
    }
    //
    ...
    // by index in future.
    //
    g_ppsHIDDevices[ulIndex] = psDevice;
    ...
    //
    // Slot the client's HID descriptor into our standard config descriptor.
    //
    ...
    //
    // If necessary, remove the interrupt OUT endpoint from the config
    // descriptor.
    //
...
//! instance may not me used in any other call to the HID device other than
//! USBDHIDInit().
//!
//! \return None.
//
...
USBDHIDTerm(void *psDevice)
{
    ...
    //
    g_ppsHIDDevices[USB_BASE_TO_INDEX(psInst->ulUSBBase)] =
                                                        (tUSBDHIDDevice *)0;
    psInst->ulUSBBase = 0;
...
//! ensure that the psDevice structure passed to USBDHIDInit() resides in
...
//*****************************************************************************
void *
USBDHIDSetRxCBData(void *psDevice, void *pvCBData)
{
    void *pvOldValue;
    ...
    // previous value.
    //
    pvOldValue = ((tUSBDHIDDevice *)psDevice)->pvRxCBData;
    ((tUSBDHIDDevice *)psDevice)->pvRxCBData = pvCBData;
...
//*****************************************************************************
void *
USBDHIDSetTxCBData(void *psDevice, void *pvCBData)
{
    void *pvOldValue;
    ...
    // previous value.
    //
    pvOldValue = ((tUSBDHIDDevice *)psDevice)->pvTxCBData;
    ((tUSBDHIDDevice *)psDevice)->pvTxCBData = pvCBData;
...
//*****************************************************************************
unsigned long
USBDHIDReportWrite(void *psDevice, unsigned char *pcData,
                   unsigned long ulLength, tBoolean bLast)
{
    ...
    if(ulLength)
    {
        ClearReportTimer(psDevice, *pcData);
    }
...
//*****************************************************************************
unsigned long
USBDHIDPacketRead(void *psDevice, unsigned char *pcData,
                  unsigned long ulLength, tBoolean bLast)
{
    ...
    //
    ulEPStatus = USBEndpointStatus(psInst->ulUSBBase, INT_OUT_ENDPOINT);
    if(ulEPStatus & USB_DEV_RX_PKT_RDY)
    {
        //
        // How many bytes are available for us to receive?
        //
        ulPkt = USBEndpointDataAvail(psInst->ulUSBBase, INT_OUT_ENDPOINT);

        //
        // Get as much data as we can.
        //
        ulCount = ulLength;
        iRetcode = USBEndpointDataGet(psInst->ulUSBBase, INT_OUT_ENDPOINT,
                                      pcData, &ulCount);
        ...
        if(ulCount == ulPkt)
        {
            ...
            // waiting.
            //
            USBDevEndpointStatusClear(psInst->ulUSBBase, INT_OUT_ENDPOINT,
                                      ulEPStatus);
            ...
            // next packet.
            //
            USBDevEndpointDataAck(psInst->ulUSBBase, INT_OUT_ENDPOINT,
                                  true);
...
//*****************************************************************************
unsigned long
USBDHIDTxPacketAvailable(void *psDevice)
{
    tHIDInstance *psInst;
...
//*****************************************************************************
unsigned long
USBDHIDRxPacketAvailable(void *psDevice)
{
    unsigned long ulEPStatus;
    unsigned long ulSize;
    tHIDInstance *psInst;
    ...
    if(ulEPStatus & USB_DEV_RX_PKT_RDY)
    {
        ...
        //
        ulSize = USBEndpointDataAvail(psInst->ulUSBBase, INT_OUT_ENDPOINT);
...
//!
//! \param psDevice is the pointer to the HID device instance structure.
...
//*****************************************************************************
void
USBDHIDPowerStatusSet(void *psDevice, unsigned char ucPower)
{
    ASSERT(psDevice);
...
//*****************************************************************************
tBoolean
USBDHIDRemoteWakeupRequest(void *psDevice)
{
    ASSERT(psDevice);

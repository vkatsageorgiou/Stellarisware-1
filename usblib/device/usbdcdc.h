...
typedef struct
{
    ...
    //
    //! A pointer to RAM workspace for this device instance.  The client must
    //! fill in this field with a pointer to a structure of type
    //! tCDCSerInstance.  This is read/write storage that the library can use
    //! for driver workspace.  This memory must remain accessible for as long
    //! as the CDC device is in use and must not be modified by any code
    //! outside the CDC class driver.
    //
...
extern void USBDCDCTerm(void *psDevice);
extern void *USBDCDCSetControlCBData(void *psDevice, void *pvCBData);
extern void *USBDCDCSetRxCBData(void *psDevice, void *pvCBData);
extern void *USBDCDCSetTxCBData(void *psDevice, void *pvCBData);
extern unsigned long USBDCDCPacketWrite(void *psDevice,
                                        unsigned char *pcData,
                                        unsigned long ulLength,
                                        tBoolean bLast);
extern unsigned long USBDCDCPacketRead(void *psDevice,
                                       unsigned char *pcData,
                                       unsigned long ulLength,
                                       tBoolean bLast);
extern unsigned long USBDCDCTxPacketAvailable(void *psDevice);
extern unsigned long USBDCDCRxPacketAvailable(void *psDevice);
extern void USBDCDCSerialStateChange(void *psDevice,
                                     unsigned short usState);
extern void USBDCDCPowerStatusSet(void *psDevice, unsigned char ucPower);
extern tBoolean USBDCDCRemoteWakeupRequest(void *psDevice);

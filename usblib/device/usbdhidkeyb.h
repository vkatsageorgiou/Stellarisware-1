...
typedef struct
{
    ...
    //! tHIDKeyboardInstance.  This is used as the read/write storage that the
    //! library can use for driver workspace.  This memory must remain
    //! accessible for as long as the keyboard device is in use and must not be
    //! modified by any code outside the HID keyboard driver.
    //
...
//! released, the device will report a rollover error to the host each time it
...
extern void USBDHIDKeyboardTerm(void *psDevice);
extern void *USBDHIDKeyboardSetCBData(void *psDevice, void *pvCBData);
extern unsigned long USBDHIDKeyboardKeyStateChange(void *psDevice,
                                                   unsigned char ucModifiers,
                                                   unsigned char ucUsageCode,
                                                   tBoolean bPressed);
extern void USBDHIDKeyboardPowerStatusSet(void *psDevice,
                                          unsigned char ucPower);
extern tBoolean USBDHIDKeyboardRemoteWakeupRequest(void *psDevice);

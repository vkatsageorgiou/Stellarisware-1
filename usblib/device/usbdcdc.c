...
// must be sized to accomodate the largest request structure that we intend
// processing.
//
//*****************************************************************************
...
#define DATA_OUT_ENDPOINT       USB_EP_3
...
//
//*****************************************************************************
static const tUSBDCDCDevice *g_ppsCDCDevices[1];
...
unsigned char g_pCDCSerDescriptor[] =
{
    ...
    USBShort(53),               // The total size of this full structure.
...
const tConfigSection g_sCDCSerInterfaceSection =
{
...
static void HandleEP0Data(unsigned long ulIndex, unsigned long ulDataSize);
...
SendSerialState(const tUSBDCDCDevice *psDevice)
{
    ...
    //
    iRetcode = USBEndpointDataPut(psInst->ulUSBBase, CONTROL_ENDPOINT,
    ...
    if(iRetcode != -1)
    {
        ...
        // sent.
        //
        iRetcode = USBEndpointDataSend(psInst->ulUSBBase, CONTROL_ENDPOINT,
        USB_TRANS_IN);
    }
                          ...
                          unsigned long ulStatus)
{
    ...
    // Get the endpoint status to see why we were called.
    //
    ulEPStatus = USBEndpointStatus(psInst->ulUSBBase, CONTROL_ENDPOINT);

    //
    // Clear the status bits.
    //
    USBDevEndpointStatusClear(psInst->ulUSBBase, CONTROL_ENDPOINT, ulEPStatus);
...
HandleEndpoints(unsigned long ulIndex, unsigned long ulStatus)
{
    ...
    if(ulStatus & USB_INTEP_DEV_IN_1)
    {
        ...
        //
        ProcessNotificationToHost(g_ppsCDCDevices[ulIndex], ulStatus);
    ...
    //
    if(ulStatus & USB_INTEP_DEV_OUT_3)
    {
        //
        // Data is being sent to us from the host.
        //
        ProcessDataFromHost(g_ppsCDCDevices[ulIndex], ulStatus);
        ulStatus &= ~USB_INTEP_DEV_OUT_3;
    }

    //
    // Handler for the bulk IN data endpoint.
    //
    if(ulStatus & USB_INTEP_DEV_IN_2)
    {
        ProcessDataToHost(g_ppsCDCDevices[ulIndex], ulStatus);
        ulStatus &= ~USB_INTEP_DEV_IN_2;
    }
}
...
HandleConfigChange(unsigned long ulIndex, unsigned long ulInfo)
{
    ...
    // Get a pointer to our instance data.
    //
    psInst = g_ppsCDCDevices[ulIndex]->psPrivateCDCSerData;
    ...
    if(!psInst->bConnected)
    {
        ...
        //
        g_ppsCDCDevices[ulIndex]->pfnControlCallback(
                                     g_ppsCDCDevices[ulIndex]->pvControlCBData,
...
//*****************************************************************************
static void
HandleEP0Data(unsigned long ulIndex, unsigned long ulDataSize)
{
    tCDCSerInstance *psInst;
    tBoolean bRetcode;
    ...
    switch (psInst->ucPendingRequest)
    {
        ...
        case USB_CDC_SET_LINE_CODING:
        {
            ...
            else
            {
                ...
                //
                bRetcode = CheckAndSendLineCodingChange(
                                                    g_ppsCDCDevices[ulIndex]);
...
HandleRequests(unsigned long ulIndex, tUSBRequest *pUSBRequest)
{
    ...
    switch (pUSBRequest->bRequest)
    {
        ...
        case USB_CDC_GET_LINE_CODING:
        {
            ...
            //
            USBDCDSendDataEP0(ulIndex, (unsigned char *)&sLineCoding,
                              sizeof(tLineCoding));
        ...
        case USB_CDC_SET_CONTROL_LINE_STATE:
        {
            ...
            //
            bRetcode = CheckAndSendLineStateChange(g_ppsCDCDevices[ulIndex]);
        ...
        case USB_CDC_SEND_BREAK:
        {
            ...
            // processed.
            //
            bRetcode = CheckAndSendBreak(g_ppsCDCDevices[ulIndex],
                                         pUSBRequest->wValue);
...
//*****************************************************************************
static void
CDCTickHandler(unsigned long ulIndex, unsigned long ulTimemS)
{
    tBoolean bCanSend;
    tCDCSerInstance *psInst;
    unsigned long ulSize;
    ...
    if(psInst->usDeferredOpFlags)
    {
        ...
        if(psInst->usDeferredOpFlags & (1 << CDC_DO_CLEAR_BREAK))
        {
            ...
            if(psInst->usBreakDuration <= ulTimemS)
            {
                ...
                //
                SendBreak(g_ppsCDCDevices[ulIndex], false);
            }
            else
            {
                //
        ...
        //
        bCanSend = DeviceConsumedAllData(g_ppsCDCDevices[ulIndex]);
        ...
        if(bCanSend)
        {
            ...
            {
                SendBreak(g_ppsCDCDevices[ulIndex], true);
            }
            ...
            {
                SendLineStateChange(g_ppsCDCDevices[ulIndex]);
            }
            ...
            {
                SendLineCodingChange(g_ppsCDCDevices[ulIndex]);
            }
        ...
        if(!psInst->bRxBlocked)
        {
            ...
            if(psInst->usDeferredOpFlags & (1 << CDC_DO_PACKET_RX))
            {
                ...
                //
                g_ppsCDCDevices[ulIndex]->pfnRxCallback(
                                          g_ppsCDCDevices[ulIndex]->pvRxCBData,
                                          USB_EVENT_RX_AVAILABLE, ulSize,
                                          (void *)0);
            }
        }
...
USBDCDCInit(unsigned long ulIndex, const tUSBDCDCDevice *psCDCDevice)
{
    ...
    ASSERT(ulIndex == 0);
    ASSERT(psCDCDevice);
    ASSERT(psCDCDevice->ppStringDescriptors);
    ...
    // controller.
    //
    if(g_ppsCDCDevices[ulIndex] != (const tUSBDCDCDevice *)0)
    {
        return ((void *)0);
    }
    //
    ...
    // by index in future.
    //
    g_ppsCDCDevices[ulIndex] = psCDCDevice;
...
//! instance may not me used in any other call to the CDC device other than
//! USBDCDCInit().
//!
//! \return None.
//
...
USBDCDCTerm(void *psDevice)
{
    ...
    // Get a pointer to our instance data.
    //
    psInst = ((tUSBDCDCDevice *)psDevice)->psPrivateCDCSerData;
    ...
    //
    g_ppsCDCDevices[USB_BASE_TO_INDEX(psInst->ulUSBBase)] =
                                                   (const tUSBDCDCDevice *)0;
    psInst->ulUSBBase = 0;
...
//*****************************************************************************
void *
USBDCDCSetControlCBData(void *psDevice, void *pvCBData)
{
    void *pvOldValue;
    ...
    //
    // Set the callback pointter for the control channel after remembering the
    // previous value.
    //
    pvOldValue = ((tUSBDCDCDevice *)psDevice)->pvControlCBData;
    ((tUSBDCDCDevice *)psDevice)->pvControlCBData = pvCBData;
...
//*****************************************************************************
void *
USBDCDCSetRxCBData(void *psDevice, void *pvCBData)
{
    void *pvOldValue;
    ...
    // previous value.
    //
    pvOldValue = ((tUSBDCDCDevice *)psDevice)->pvRxCBData;
    ((tUSBDCDCDevice *)psDevice)->pvRxCBData = pvCBData;
...
//*****************************************************************************
void *
USBDCDCSetTxCBData(void *psDevice, void *pvCBData)
{
    void *pvOldValue;
    ...
    // previous value.
    //
    pvOldValue = ((tUSBDCDCDevice *)psDevice)->pvTxCBData;
    ((tUSBDCDCDevice *)psDevice)->pvTxCBData = pvCBData;
...
//*****************************************************************************
unsigned long
USBDCDCPacketWrite(void *psDevice, unsigned char *pcData,
...
//*****************************************************************************
unsigned long
USBDCDCPacketRead(void *psDevice, unsigned char *pcData,
                  unsigned long ulLength, tBoolean bLast)
{
...
//*****************************************************************************
unsigned long
USBDCDCTxPacketAvailable(void *psDevice)
{
    tCDCSerInstance *psInst;
...
//*****************************************************************************
unsigned long
USBDCDCRxPacketAvailable(void *psDevice)
{
    unsigned long ulEPStatus;
    unsigned long ulSize;
    tCDCSerInstance *psInst;
...
//*****************************************************************************
void
USBDCDCSerialStateChange(void *psDevice, unsigned short usState)
{
    tCDCSerInstance *psInst;
    ...
    //
    // 1.  Errors (overrun, partiy, framing error) are ORed together so that
...
//!
//! \param psDevice is the pointer to the CDC device instance structure.
...
//*****************************************************************************
void
USBDCDCPowerStatusSet(void *psDevice, unsigned char ucPower)
{
    ASSERT(psDevice);
...
//*****************************************************************************
tBoolean
USBDCDCRemoteWakeupRequest(void *psDevice)
{
    ASSERT(psDevice);

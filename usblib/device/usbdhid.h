...
typedef struct
{
    ...
    //! USB_HID_WORKSPACE_SIZE bytes of read/write storage that the library
    //! can use for driver workspace.  This memory must remain accessible for
    //! as long as the HID device is in use and must not be modified by any
    //! code outside the HID class driver.
    //
...
extern void USBDHIDTerm(void *psDevice);
extern void *USBDHIDSetRxCBData(void *psDevice, void *pvCBData);
extern void *USBDHIDSetTxCBData(void *psDevice, void *pvCBData);
extern unsigned long USBDHIDReportWrite(void *psDevice,
                                        unsigned char *pcData,
                                        unsigned long ulLength,
                                        tBoolean bLast);
extern unsigned long USBDHIDPacketRead(void *psDevice,
                                       unsigned char *pcData,
                                       unsigned long ulLength,
                                       tBoolean bLast);
extern unsigned long USBDHIDTxPacketAvailable(void *psDevice);
extern unsigned long USBDHIDRxPacketAvailable(void *psDevice);
extern void USBDHIDPowerStatusSet(void *psDevice, unsigned char ucPower);
extern tBoolean USBDHIDRemoteWakeupRequest(void *psDevice);

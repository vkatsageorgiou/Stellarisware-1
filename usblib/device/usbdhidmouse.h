...
typedef struct
{
    ...
    //! tHIDMouseInstance.  This is used as the read/write storage that the
    //! library can use for driver workspace.  This memory must remain
    //! accessible for as long as the mouse device is in use and must not be
    //! modified by any code outside the HID mouse driver.
    //
...
extern void USBDHIDMouseTerm(void *psDevice);
extern void *USBDHIDMouseSetCBData(void *psDevice, void *pvCBData);
extern unsigned long USBDHIDMouseStateChange(void *psDevice, char cDeltaX,
                                             char cDeltaY,
                                             unsigned char ucButtons);
extern void USBDHIDMousePowerStatusSet(void *psDevice,
                                       unsigned char ucPower);
extern tBoolean USBDHIDMouseRemoteWakeupRequest(void *psDevice);

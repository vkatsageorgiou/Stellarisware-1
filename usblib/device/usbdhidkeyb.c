...
//
//*****************************************************************************
static tUSBDHIDDevice g_sHIDKeybDevice =
{
    0,                          // Will be filled in during USBDHIDKeyboardInit
    0,                          // Will be filled in during USBDHIDKeyboardInit
    0,                          // Will be filled in during USBDHIDKeyboardInit
    0,                          // Will be filled in during USBDHIDKeyboardInit
    USB_HID_SCLASS_BOOT,
    ...
    1,
    0,                          // Will be filled in during USBDHIDKeyboardInit
    HIDKeyboardRxHandler,
    0,                          // Will be filled in during USBDHIDKeyboardInit
    HIDKeyboardTxHandler,
    0,                          // Will be filled in during USBDHIDKeyboardInit
    false,
    &g_sKeybHIDDescriptor,
    g_pKeybClassDescriptors,
    0,                          // Will be filled in during USBDHIDKeyboardInit
    0,                          // Will be filled in during USBDHIDKeyboardInit
    0                           // Will be filled in during USBDHIDKeyboardInit
};
    ...
    switch (ulEvent)
    {
        ...
        case USB_EVENT_TX_COMPLETE:
        {
            ...
            if(psInst->bChangeMade)
            {
                ...
                //
                ulCount = USBDHIDReportWrite((void *)&g_sHIDKeybDevice,
                                             psInst->pucReport,
...
//
// \return Returns \b true if the usage code wast successfully added to the
// list or \b false if there was insufficient space to hold the new key
// press (in which case the caller should report a rollover error to the host).
//
//*****************************************************************************
static tBoolean
                    ...
                    const tUSBDHIDKeyboardDevice *psDevice)
{
    ...
    // the caller.
    //
    g_sHIDKeybDevice.usPID = psDevice->usPID;
    g_sHIDKeybDevice.usVID = psDevice->usVID;
    g_sHIDKeybDevice.usMaxPowermA = psDevice->usMaxPowermA;
    g_sHIDKeybDevice.ucPwrAttributes = psDevice->ucPwrAttributes;
    g_sHIDKeybDevice.ppStringDescriptors = psDevice->ppStringDescriptors;
    g_sHIDKeybDevice.ulNumStringDescriptors = psDevice->ulNumStringDescriptors;
    g_sHIDKeybDevice.psPrivateHIDData = &psInst->sHIDInstance;
    g_sHIDKeybDevice.psReportIdle = &psInst->sReportIdle;
    g_sHIDKeybDevice.pvRxCBData = (void *)psDevice;
    g_sHIDKeybDevice.pvTxCBData = (void *)psDevice;
    ...
    //
    pvRetcode = USBDHIDInit(ulIndex, &g_sHIDKeybDevice);
...
//! psDevice instance may not me used in any other call to the HID keyboard
...
//*****************************************************************************
void
USBDHIDKeyboardTerm(void *psDevice)
{
    tHIDKeyboardInstance *psInst;
    ...
    // Get a pointer to our instance data
    //
    psInst = (((tUSBDHIDKeyboardDevice *)psDevice)->psPrivateHIDKbdData);
...
//! ensure that the psDevice structure passed to USBDHIDKeyboardInit() resides
...
//*****************************************************************************
void *
USBDHIDKeyboardSetCBData(void *psDevice, void *pvCBData)
{
    void *pvOldCBData;
    ...
    //
    psKeyboard = (tUSBDHIDKeyboardDevice *)psDevice;
...
//*****************************************************************************
unsigned long
USBDHIDKeyboardKeyStateChange(void *psDevice, unsigned char ucModifiers,
                              unsigned char ucUsageCode, tBoolean bPress)
{
    ...
    if(ucUsageCode != HID_KEYB_USAGE_RESERVED)
    {
        ...
        // and got a bad return code indicating a rollover error, we need to
    ...
    //
    if(USBDHIDTxPacketAvailable((void *)&g_sHIDKeybDevice))
    {
        //
...
//!
//! \param psDevice is the pointer to the keyboard device instance structure.
...
//*****************************************************************************
void
USBDHIDKeyboardPowerStatusSet(void *psDevice, unsigned char ucPower)
{
    ASSERT(psDevice);
    ...
    //
    USBDHIDPowerStatusSet((void *)&g_sHIDKeybDevice, ucPower);
}
...
//*****************************************************************************
tBoolean
USBDHIDKeyboardRemoteWakeupRequest(void *psDevice)
{
    ASSERT(psDevice);
    ...
    // Pass the request through to the lower layer.
    //
    return(USBDHIDRemoteWakeupRequest((void *)&g_sHIDKeybDevice));
}

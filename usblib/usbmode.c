...
//
//*****************************************************************************
volatile unsigned long ulWaitTicks = 0;
...
{
    //
    // No checking is currrently pending.
    //
    USB_OTG_MODE_IDLE,
    ...
    //
    // Now in A-side wait for connect.
    //
    USB_OTG_MODE_A_WAITCON,
...
{
    //
    // This function should not be called unless the current state is either
    // an A side host or and A side waiting for a device to connect.
    //
    if((g_eOTGModeState != USB_OTG_MODE_A_HOST) &&
       (g_eOTGModeState != USB_OTG_MODE_A_WAITCON))
    {
        return;
    }
    ...
    //
    // Disable power so that new detection can take place.
    //
...
USB0OTGModeIntHandler(void)
{
    ...
    //
    // Check if this was an mode detect interrupt.
    //
    if(ulStatus & USB_INTCTRL_MODE_DETECT)
    {
        switch(USBModeGet(USB0_BASE))
        {
            //
            ...
            {
                //
                // Now in host mode on the A side of the cable.
                //
                g_eOTGModeState = USB_OTG_MODE_A_WAITCON;

                //
                // Power must be enabled or a VBUS error will occur.
                //
    ...
    //
    // If there is a connect interrupt while the library is waiting for one
    // then move to host mode and start allowing the host stack to handler
    // interrupts.
    //
...
{
    if(ulMsTicks > ulWaitTicks)
    {
        ulWaitTicks = 0;
    }
    else
    {
        ulWaitTicks -= ulMsTicks;
    }
    ...
    switch(g_eOTGModeState)
    {
        ...
        case USB_OTG_MODE_WAITID:
        {
            ...
            //
            if((ulWaitTicks == 0) && (g_ulPollRate != 0))
            {
                //
                // Return to unconfigured mode.
                //
                USBOTGSetMode(USB_MODE_NONE);

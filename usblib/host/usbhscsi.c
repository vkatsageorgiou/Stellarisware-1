                    ...
                    unsigned long *pulSize)
{
    ...
    // Send the command.
    //
    USBHCDPipeWrite(ulOutPipe, (unsigned char*)pSCSICmd, sizeof(tMSCCBW));
    ...
    if(pSCSICmd->dCBWDataTransferLength != 0)
    {
        ...
        if(pSCSICmd->bmCBWFlags & CBWFLAGS_DIR_IN)
        {
            ...
            // Read the data back.
            //
            ulBytes = USBHCDPipeRead(ulInPipe, pucData, *pulSize);

            //
            // Caculate the bytes read.
            //
            *pulSize = ulBytes;
        ...
        else
        {
            ...
            // Write the data out.
            //
            ulBytes = USBHCDPipeWrite(ulOutPipe, pucData, *pulSize);

            //
            // Caculate the bytes written.
            //
            *pulSize = ulBytes;
    ...
    //
    USBHCDPipeRead(ulInPipe, (unsigned char *)&CmdStatus, sizeof(tMSCCSW));

    //
    // If the status was invalid or did not have the correct signature then
    // indicate a failure.
    //
    if((CmdStatus.dCSWSignature != CSW_SIGNATURE) ||
               ...
               unsigned long *pulSize, unsigned long ulNumBlocks)
{
    ...
    //
    *((unsigned short *)&SCSICmd.CBWCB[7]) = (ulNumBlocks & 0xFF00) >> 8;
    *((unsigned short *)&SCSICmd.CBWCB[8]) = (ulNumBlocks & 0xFF);

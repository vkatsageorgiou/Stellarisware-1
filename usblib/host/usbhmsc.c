           ...
           USB_EP_ATTR_BULK)
        {
            ...
            if(pEndpointDescriptor->bEndpointAddress & USB_EP_DESC_IN)
            {
                ...
                g_USBHMSCDevice.ulBulkInPipe =
                    USBHCDPipeAlloc(0, USBHCD_PIPE_BULK_IN_DMA,
                                    pDevice->ulAddress, 0);
                //
            ...
            else
            {
                ...
                g_USBHMSCDevice.ulBulkOutPipe =
                    USBHCDPipeAlloc(0, USBHCD_PIPE_BULK_OUT_DMA,
                                    pDevice->ulAddress, 0);
                //
...
USBHMSCDriveReady(unsigned long ulInstance)
{
    ...
    unsigned long ulSize, ulTimeout;
    ...
    //
    ulSize = SCSI_INQUIRY_DATA_SZ;
    if(USBHSCSIReadCapacity(pMSCDevice->ulBulkInPipe,
       ...
       SCSI_CMD_STATUS_PASS)
    {
        ...
        //
        // Initialize the timeout for the next loop.
        //
        ulTimeout = 0;

        //
        // If the error was that the device was not present then wait around to
        // see if it becomes ready before failing this call.
        //
        while(pBuffer[SCSI_RS_SKEY_AD_SKEY] == SCSI_RS_KEY_NOTPRSNT)
        {
            //
            // Delay 10ms give the device some time to recover.
            //
            SysCtlDelay(SysCtlClockGet() / 100);

            //
            // Send a SCSI Request Sense to see if the device status has
            // changed.
            //
            ulSize = SCSI_INQUIRY_DATA_SZ;
            USBHSCSIRequestSense(pMSCDevice->ulBulkInPipe,
            ...
            //
            // Time out because the device is not ready yet.
            //
            if(ulTimeout++ >= 20)
            {
                return(-1);
        ...
        //
        // See if the device still requires attention.
        //
        if((pBuffer[SCSI_RS_SKEY] & SCSI_RS_KEY_M) == SCSI_RS_KEY_UNIT_ATTN)
        {
            //
            // Return a failure since the device still requires attention.
            //
            return(-1);
        }

        //
        // Otherwise, see if the device claims this is an illegal request.
        //
        else if((pBuffer[SCSI_RS_SKEY] & SCSI_RS_KEY_M) ==
                SCSI_RS_KEY_ILGL_RQST)
        {
            //
            // Return a failure since the device claims this is an illegal
            // request (which is rare).
            //
            return(-1);
        }
    }

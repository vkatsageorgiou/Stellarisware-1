                     ...
                     tBoolean *pbChannels, tRendererMeasurements *psMeasure)
{
    ...
    //
    GrContextFontSet(&g_sOffscreenContext, &g_sFontFixed6x8);
...
// the application.  The screen fades from black to white then the Luminary
// Micro logo fades down from white.  Once this fade is completed, the third
// party toolchain vendor logo is shown at the bottom of the screen.  We wait
// a couple of seconds then clear the screen to black and return.
//
//*****************************************************************************
void
RendererShowStartupScreen(void)
{
    ...
    //
    // Determine where on the screen the Luminary logo is going to go.  We
    // center it horizontally and set the vertical position to leave sufficient
    // space to place the toolchain logo beneath it.
    //
    lX = ((DpyWidthGet(&g_sFormike128x128x16) -
           GrImageWidthGet(g_pucLuminaryMicroImage)) / 2);

    if(GrImageWidthGet(g_pucToolchainLogoImage) == 0)
    {
        //
        // The application has been built without a special toolchain logo
        // so we just center the Luminary Micro logo on the screen.
        //
        lY = ((DpyHeightGet(&g_sFormike128x128x16) -
               GrImageHeightGet(g_pucLuminaryMicroImage)) / 2);
    }
    else
    {
        //
        // We have to fit a toolchain logo at the bottom of the screen after
        // we fade in the Luminary Micro logo.  Position the first logo such
        // that we have equal amounts of whitespace above, below and between
        // the two logos.
        //
        lY = ((DpyHeightGet(&g_sFormike128x128x16) -
               (GrImageHeightGet(g_pucLuminaryMicroImage) +
                GrImageHeightGet(g_pucToolchainLogoImage))) / 3);
    }

    //
    // Now fade in the Luminary Micro logo.
    //
    FadeInImage(g_pucLuminaryMicroImage, lX, lY, 3);

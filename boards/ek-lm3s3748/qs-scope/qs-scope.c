...
//! and via download from the Luminary Micro web site at
//! http://www.luminarymicro.com/products/software_updates.html.  This
...
// signalled.  It polls the current state of the buttons and performs any
...
TestSignalInit(void)
{
    ...
    //
    ROM_PWMGenConfigure(PWM_BASE, PWM_GEN_0, (PWM_GEN_MODE_DOWN |
                        PWM_GEN_MODE_DBG_RUN | PWM_GEN_MODE_NO_SYNC));
    ROM_PWMGenPeriodSet(PWM_BASE, PWM_GEN_0, (ROM_SysCtlClockGet() / 1000));
    ROM_PWMPulseWidthSet(PWM_BASE, PWM_OUT_0, (ROM_SysCtlClockGet() / 2000));
...
//
// \return Returns \b true if a display update is requried, \b false if not.
//
//*****************************************************************************
tBoolean
...
// merely by cancelling the outstanding acquisition request, we would end up
...
main(void)
{
    ...
    //
    //
    // Are we operating as a USB host or device?
    //
    if(g_bUSBModeIsHost)
    {
        //
        // Initialize as a host and start looking for a Mass Storage Class
    ...
    {
        //
        // Initialize as a device and publish our descriptors.
        //
        ScopeUSBDeviceInit();
    }

...
//*****************************************************************************
//
// This function is called from HandleEndpoints for all interrupts signalling
...
//****************************************************************************
//
// This function configures the oscilloscope USB device endpoints and
// allocates the required amount of USB controller FIFO space to support
// them.
//
//****************************************************************************
void
ScopeUSBDeviceConfigureEndpoints(void)
{
    unsigned long ulFIFOAddr;

    //
    // Configure the data IN endpoint (on which we send data to the host).
    //
    USBDevEndpointConfigSet(USB0_BASE, DATA_IN_ENDPOINT, DATA_IN_EP_MAX_SIZE,
                            USB_EP_DEV_IN | USB_EP_MODE_BULK);

    //
    // Configure the data OUT endpoint (on which we receive data from the
    // host).
    //
    USBDevEndpointConfigSet(USB0_BASE, DATA_OUT_ENDPOINT, DATA_OUT_EP_MAX_SIZE,
                            USB_EP_DEV_OUT | USB_EP_MODE_BULK);

    //
    // Configure the USB FIFO.  Note that we are responsible for providing both
    // the start address and size of each FIFO block so we need to make sure
    // none of these blocks overlap.  Endpoint 0 takes the first 64 bytes of
    // the address range so we start there.
    //
    ulFIFOAddr = 64;

    //
    // Configure data IN endpoint FIFO.
    //
    USBFIFOConfigSet(USB0_BASE, DATA_IN_ENDPOINT, ulFIFOAddr,
                     DATA_IN_EP_FIFO_SIZE, USB_EP_DEV_IN);
    ulFIFOAddr += USB_FIFO_SZ_TO_BYTES(DATA_IN_EP_FIFO_SIZE);

    //
    // Configure data OUT endpoint FIFO.
    //
    USBFIFOConfigSet(USB0_BASE, DATA_OUT_ENDPOINT, ulFIFOAddr,
                     DATA_OUT_EP_FIFO_SIZE, USB_EP_DEV_OUT);
    ulFIFOAddr += USB_FIFO_SZ_TO_BYTES(DATA_OUT_EP_FIFO_SIZE);
...
// resynchronise with the host.  We clean up all state and revert to
...
// no longer being communicated with and reinitiate communications after this.
//
//*****************************************************************************
static void
...
HandleEndpoints(unsigned long ulIndex, unsigned long ulStatus)
{
    ...
    //
    // Check to see if any other interrupt causes are being signalled.  If
